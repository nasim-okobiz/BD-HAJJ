...
class AboutUsService extends BaseService {
  ...
  async createAboutUs(payloadFiles, payload, session) {
    ...
    const { header, title,
      ...
      honorName, whatsApp,
      email } = payload;
    if (!header || !title || !description || !agencyName || !honorName || !whatsApp || !email) {
    ...
    if (Object.values(requiredFiles).some(required => !required)) {
      throw new Error("All required photos or documents must be provided: photo, background Photo.");
    }
    let images;
  ...
  async updateAboutUs(id, payloadFiles, payload) {
    const { files } = payloadFiles;
    const { header, title, description, agencyName, honorName, whatsApp, email } = payload;
