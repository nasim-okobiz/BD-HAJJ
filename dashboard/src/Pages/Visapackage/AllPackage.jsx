// src/components/VisaPackagesComponent.js
...
const VisaPackagesComponent = () => {
  ...
  const fetchPackages = async () => {
    try {
      const response = await axiosInstance.get("/package?priority=3");
  ...
  const columns = [
...

    {
      ...
      dataIndex: "hotelDistance", // Corrected key
      key: "hotelDistance", // Corrected key
      render: (text) => (
        <div className="space-y-2">
          {Array.isArray(text) && text.length > 0 ? (
            text.map((distance, index) => (
            ...
            <span>No distances available</span>
          )}
        </div>
      ),
    },
...
export default VisaPackagesComponent;
